...
export function Skills() {
  ...
  return (
    ...
    <section name="skills" className="skills container">
      <div className="title">
        {currentLanguage === LANGUAGES[0] ? 'Навыки' : 'Skills'}
      </div>
      <div className="skills__wrap">
        {skills.map((skill) => {
          return (
            <div
              className={`skill__circle animate__animated ${
                activeSection === SECTION_LINKS.skills
                  ? `${skill.animationIn} flex`
                  : ''
              } ${
                inactiveSection === SECTION_LINKS.skills
                  ? `${skill.animationOut} flex`
                  : ''
              }`}
              key={skill.icon}
            >
              <FontAwesomeIcon
                className={`skill ${skill.row} ${skill.column}`}
                icon={`fa-brands ${skill.icon}`}
              />
            </div>
          );
        })}
