...
//решила показывать проекты от последнего к первому
...
//на самом деле это reversed projects, но тогда бы пришлось везде переименовывать
const projects = unreversedProjects.reverse();
...
export function Portfolio() {
  ...
  useEffect(() => {
    if (infinite) {
      setItems([projects[projects.length - 1], ...projects, projects[0]]);
  ...
  return (
    <section name="portfolio" className="portfolio container">
      <div className="title">
        {currentLanguage === LANGUAGES[0] ? 'Портфолио' : 'Portfolio'}
      </div>
